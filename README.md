...
### 为什么微备份在Web端不显示账号，但在飞牛app手机端可以正常显示？

微备份Web端不显示账号，但在飞牛app手机端显示，重装微备份，也重启过Nas设备，还是无法解决（如下图所示）。关于这个问题目前还不知道是什么原因造成的，但反馈的用户基本是使用了微软自带的浏览器Edge。目前的临时解决方案是：`Edge->设置->隐私、搜索和服务->Cookie->所有 Cookie 和站点数据->找到你飞牛的IP或者域名，清理，然后刷新浏览器，重新登录飞牛账号`
![1789972466720](assets/1789972466720.png)
![1789972384911](assets/1789972384911.png)
...
### ● 如果微信号被封了，无法在新Nas设备、新硬盘、新系统上登录，怎么办？
...
2、在新设备的微备份登录你已经备份过的微信号，并且在“设置-备份路径”设置新的备份路径（如果微信号被封了，使用终端工具，在旧的Nas设备把这个 `/var/apps/WxBackup/var/dbs/` 文件夹下的 **global.db、 global.db-shm、global.db-wal** 三个文件 复制到 新Nas设备的`/var/apps/WxBackup/var/dbs/` 文件夹下，覆盖新Nas设备上的 **global.db、 global.db-shm、global.db-wal** 三个文件）
